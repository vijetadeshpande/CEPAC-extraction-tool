...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 16:40:08 2020
...
for strategy in strategies:   
    ...
    float_name = 'TWSA_var1=' + var_list[0] + '_var2=' + var_list[1]
    readpaths = os.path.join(readbase, float_name)
    finalpaths = os.path.join(writebase, 'Final runs_var1=%s_var2=%s'%(var_list[0], var_list[1]))
    ...
    # collect output
    try:
        c_op.collect_output(readpaths)
    except:
        pass
    
    # read output
    cepac_outputs['var'] = link.import_all_cepac_out_files(os.path.join(readpaths, 'results'), module = 'regression')
    ...
    for file in cepac_outputs['var']:
        ...
        filepath = os.path.join(readpaths, file) + '.in'
        ...
        # write the file
        val1, val2 = file.split('=')[1].split('_')[0], file.split('=')[2]
        addstr = 'TWSA_final_run_%s=%s_%s=%s'%(var_list[0], val1, var_list[1], val2) + '.in'
        if not os.path.exists(finalpaths): os.makedirs(finalpaths)
        savepath = os.path.join(finalpaths, addstr)
        link.write_cepac_in_file(savepath, float_in)
    
    # parallelize
    c_op.parallelize_input(finalpaths)
